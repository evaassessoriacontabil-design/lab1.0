...
exports.handler = async (event) => {
  ...
  try{
    ...
    if (!token || token.split('.').length !== 3) throw new Error('missing');

    const [h,p,s] = token.split('.');
    const secret = process.env.ACCESS_TOKEN_SECRET;
    const expect = signHS256(`${h}.${p}`, secret);
    if (s !== expect) throw new Error('bad-sign');

    const payload = JSON.parse(fromB64u(p));
    const now = Math.floor(Date.now()/1000);
    if (payload.exp && payload.exp < now) throw new Error('expired');

    return { statusCode:200, headers:{ 'Content-Type':'application/json', 'Access-Control-Allow-Origin':origin }, body: JSON.stringify({ valido:true, email: payload.email || '' }) };
  }catch(e){
    return { statusCode:200, headers:{ 'Content-Type':'application/json', 'Access-Control-Allow-Origin':origin }, body: JSON.stringify({ valido:false }) };
  }
};
